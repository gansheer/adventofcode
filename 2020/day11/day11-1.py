...
def execute_round_seat(old_ferry_map, r_index, c_index):
  ...
  adjacent_occupied_seats = sum(map(lambda value: 1 if value == '#' else 0, adjacent_seats))
  ...
  if seat == 'L' and adjacent_occupied_seats == 0:
    return '#'
  if seat == '#' and adjacent_occupied_seats >= 4:
    return 'L'
    ...
    for dc in (-1, 0, +1):
      if not (dr == 0 and dc == 0):
        ...
        if new_r >= 0 and new_r < max_r and new_c >= 0 and new_c < max_c:
          #print(new_r,new_c)
          #print(ferry_map[new_r])
          adjacent_seats.append(ferry_map[new_r][new_c])
