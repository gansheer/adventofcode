...
func applyMove(move Move, cargoCrane CargoCrane) CargoCrane {
	...
	for i := 0; i < count; i++ {
		...
		objectToMove := cargoCrane.crates[move.source][crateSize-1]
		fmt.Println(crateSize, objectToMove)
		fmt.Println(cargoCrane.crates[move.source])
		fmt.Println(cargoCrane.crates[move.target])
		cargoCrane.crates[move.source] = cargoCrane.crates[move.source][0 : crateSize-1]
		cargoCrane.crates[move.target] = append(cargoCrane.crates[move.target], objectToMove)
	}
	fmt.Println(cargoCrane)
	return cargoCrane
